...
{
    public class PurchaseInvoicePrint
    {
        ...
        public PurchaseInvoicePrint(DataTable invoiceData, DataTable itemsData)
        {
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            // 2. عنوان الفاتورة
            g.DrawString("فاتورة مشتريات", fontTitle, blackBrush, centerX, yPos, centerFormat);
            yPos += 30;
            ...
            if (dtInvoice.Rows.Count > 0)
            {
                ...
                string purchaseNumber = invoice["PurchaseNumber"].ToString();
                DateTime purchaseDate = Convert.ToDateTime(invoice["PurchaseDate"]);
                string supplierName = invoice["SupplierName"].ToString();
                string supplierInvoice = invoice["SupplierInvoiceNumber"].ToString();
                string userName = invoice["UserName"].ToString();

                // رقم الفاتورة والتاريخ
                g.DrawString($"رقم الفاتورة: {purchaseNumber}", fontBold, blackBrush, leftMargin, yPos);
                g.DrawString($"التاريخ: {purchaseDate:yyyy/MM/dd}", fontBold, blackBrush, rightMargin, yPos, rightFormat);
                yPos += 22;

                g.DrawString($"المورد: {supplierName}", fontRegular, blackBrush, leftMargin, yPos);
                g.DrawString($"فاتورة المورد: {supplierInvoice}", fontRegular, blackBrush, rightMargin, yPos, rightFormat);
                yPos += 22;

                g.DrawString($"المستخدم: {userName}", fontRegular, blackBrush, leftMargin, yPos);
                yPos += 30;
            }
            ...
            float col3 = rightMargin - 320; // الكمية
            float col4 = rightMargin - 240; // السعر
            float col5 = rightMargin - 160; // الإجمالي
            float col6 = rightMargin - 80;  // تاريخ الصلاحية
            ...
            g.DrawString("الإجمالي", fontBold, blackBrush, col5, yPos);
            g.DrawString("الصلاحية", fontBold, blackBrush, col6, yPos);
            yPos += 22;
            ...
            for (int i = currentItemIndex; i < dtItems.Rows.Count; i++)
            {
                ...
                string expiryDate = item["ExpiryDate"] != DBNull.Value
                    ? Convert.ToDateTime(item["ExpiryDate"]).ToString("yyyy/MM/dd")
                    : "---";
                ...
                g.DrawString(total.ToString("N2"), fontRegular, blackBrush, col5, yPos);
                g.DrawString(expiryDate, fontSmall, blackBrush, col6, yPos);
            ...
            g.DrawString("نظام إدارة المستودعات الطبية", fontRegular, grayBrush, centerX, yPos, centerFormat);
            yPos += 20;
            g.DrawString("هذه الفاتورة تمت طباعتها إلكترونياً",
                fontSmall, grayBrush, centerX, yPos, centerFormat);
